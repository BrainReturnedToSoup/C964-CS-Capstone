...
class ModelAssets:
    def __init__(self):
        self.pretrained_gradient_boosted_regressor:GradientBoostingRegressor=joblib.load(path / "model.pkl")
        self.prefit_scaler:StandardScaler=joblib.load(path / "scaler.pkl")
        
        self.dataset_df=pd.read_csv(path / "housing_price_dataset.csv")
        
        self.trainX_raw_df=pd.DataFrame(joblib.load(path / "trainX.pkl"))
        self.trainX_transformed_df=pd.DataFrame(joblib.load(path / "trainX_renamed_scaled_ordered.pkl"))
        self.trainY_df=pd.DataFrame(joblib.load(path / "trainY.pkl")).loc[self.trainX_raw_df.index]
        
        self.testX_raw_df=pd.DataFrame(joblib.load(path / "testX.pkl"))
        self.testX_transformed_df=pd.DataFrame(joblib.load(path / "testX_renamed_scaled_ordered.pkl"))
        self.testY_df=pd.DataFrame(joblib.load(path / "testY.pkl")).loc[self.testX_raw_df.index]
    
        self.deltasY=pd.Series(joblib.load(path / "deltasY.pkl"), index=self.testX_raw_df.index)
